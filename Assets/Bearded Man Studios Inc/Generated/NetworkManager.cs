...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] TestPlayerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is TestPlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (TestPlayerNetworkObject.Length > 0 && TestPlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(TestPlayerNetworkObject[obj.CreateCode]);
		...
		[Obsolete("Use InstantiateTestPlayer instead, its shorter and easier to type out ;)")]
		public TestPlayerBehavior InstantiateTestPlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(TestPlayerNetworkObject[index]);
		...
		/// <summary>
		/// Instantiate an instance of TestPlayer
		/// </summary>
		/// <returns>
		/// A local instance of TestPlayerBehavior
		/// </returns>
		/// <param name="index">The index of the TestPlayer prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
